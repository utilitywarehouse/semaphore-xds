...
func NewEndpointSliceWatcher(client kubernetes.Interface, resyncPeriod time.Duration, handler EndpointSliceEventHandler, labelSelector, namespace string) *EndpointSliceWatcher {
...
func (esw *EndpointSliceWatcher) Init() {
	listWatch := &cache.ListWatch{
		ListFunc: func(options metav1.ListOptions) (runtime.Object, error) {
			...
			if err != nil {
				log.Logger.Error("EndpointSlice list error", "error", err)
			} else {
			}
			return l, err
		},
		WatchFunc: func(options metav1.ListOptions) (watch.Interface, error) {
			...
			if err != nil {
				log.Logger.Error("EndpointSlice watch error", "error", err)
			} else {
			}
			return w, err
		},
