...
func TestXdsServiceStore_AddMany(t *testing.T) {
	...
	serviceStore.AddOrUpdate(svcF, clusterv3.Cluster_ROUND_ROBIN)
	svcO := &v1.Service{
		ObjectMeta: metav1.ObjectMeta{
			Name:      "other",
			Namespace: "bar",
	...
	serviceStore.AddOrUpdate(svcO, clusterv3.Cluster_ROUND_ROBIN)
...
func TestXdsServiceStore_Update(t *testing.T) {
	...
	serviceStore.AddOrUpdate(svc, clusterv3.Cluster_RING_HASH)
	assert.Equal(t, 1, len(serviceStore.All()))
	s, err = serviceStore.Get("foo", "bar")
	if err != nil {
		t.Fatal(err)
